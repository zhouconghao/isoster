"""LSB auto-lock demo on PGC006669 (LegacySurvey grz).

Sweeps three lock configurations on the same PGC006669 cutout and writes:

- ``<out_dir>/PGC006669_lsb_auto_lock_<config>.fits`` per config.
- ``<out_dir>/PGC006669_lsb_auto_lock_<config>.png`` per-config QA mosaic.
- ``<out_dir>/PGC006669_lsb_auto_lock_compare.png`` cross-config geometry
  vs SMA panel (red dashed line marks lock-fire SMA per config).
- ``<out_dir>/PGC006669_lsb_auto_lock_stats.json`` per-config summary.

Configurations match the asteris demo: baseline, lock-mean, lock-median.
PGC006669 has a real bar + outer-disc structural transition, so this
demo also doubles as a stress test for the "galaxy-type caveat"
documented in ``docs/10-multiband.md``: the lock pinning the outer
disc to the bar's geometry is exactly the failure mode users need to
recognize before enabling the lock as a default.
...
def run(galaxy_dir: Path, galaxy_prefix: str, out_dir: Path) -> None:
    ...
    for label, lock_on, integrator in LOCK_CONFIGS:
        ...
        cfg = _build_cfg(bands, maxsma, lock_on=lock_on, integrator=integrator)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", UserWarning)
        ...
        fits_path = out_dir / f"{galaxy_prefix}_lsb_auto_lock_{slug}.fits"
        isophote_results_mb_to_fits(res, str(fits_path))
        print(f"  wrote {fits_path}")

        qa_path = out_dir / f"{galaxy_prefix}_lsb_auto_lock_{slug}.png"
        plot_qa_summary_mb(
            result=res,
            ...
            title=f"PGC006669 — lsb_auto_lock config: {label}",
        )
    ...
    stats_path = out_dir / f"{galaxy_prefix}_lsb_auto_lock_stats.json"
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        default=Path("outputs/benchmark_multiband/lsb_auto_lock_pgc"),
        type=Path,
    )
