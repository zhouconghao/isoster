...
- Outer region center regularization (`use_outer_center_regularization`): the top-level result dict gains `use_outer_center_regularization` (echo), `outer_reg_x0_ref`, and `outer_reg_y0_ref` carrying the frozen inner reference centroid. No per-isophote fields are added.
...
- Outer region center regularization (`use_outer_center_regularization=True`): after the inward loop, the driver calls `_build_outer_center_reference(inwards_results, anchor_iso, cfg)` to compute a flux-weighted mean over the anchor plus qualifying inward isophotes (acceptable stop codes, `sma <= sma0 * outer_reg_ref_sma_factor`). The result `(x0_ref, y0_ref)` is carried as a separate `outer_reference_geom` kwarg into `fit_isophote` for outward calls. Inside the fitting loop, `compute_outer_center_regularization_penalty` injects `lambda(sma) * delta_r^2` into `effective_amp` in the best-iteration selector, where `lambda(sma) = outer_reg_strength / (1 + exp(-(sma - onset) / width))`. The penalty composes cleanly with the automatic LSB lock: after the lock, `fix_center=True` makes the penalty a no-op on the locked tail. Inward growth never gets the penalty (`outer_reference_geom=None` for inward calls). The feature is only wired into the regular-mode driver — template-based forced photometry emits a `UserWarning` and the feature is silently inactive. Combining it with `fix_center=True` also emits a `UserWarning` at config time because the penalty is inert when the center cannot move. It is agnostic to `use_eccentric_anomaly`, `simultaneous_harmonics`, and isofit-style modes because the penalty rides the same `effective_amp` rail in the best-iteration selector.
