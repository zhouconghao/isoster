#!/usr/bin/env python3
"""Step 1 EA-only run: eccentric anomaly sampling without simultaneous harmonics.

Uses asymmetric sigma clipping (sclip_low=3.0, sclip_high=2.0, nclip=3) with
eccentric anomaly coordinate sampling but standard post-hoc harmonic computation
(no simultaneous fitting inside the iteration loop).

Usage
-----
    uv run python examples/example_hsc_edgecases/run_step1_ea_only.py
"""
...
OUTPUT_DIR = (
    ...
    / "outputs" / "example_hsc_edgecases" / "step1_ea_only"
)
...
CONFIG_KWARGS = dict(
    ...
    sclip_low=3.0,
    sclip_high=2.0,
    # Eccentric anomaly sampling, NO simultaneous harmonics
...
def run_one_galaxy(obj_id, desc):
    """Run EA-only fitting on one galaxy."""
...
def main():
    ...
    print("Step 1: EA-only + asymmetric clipping on HSC edge cases")
    print(f"  EA=True, simultaneous_harmonics=False, compute_deviations=True")
