...
from isoster.numba_kernels import (
    ...
    _build_harmonic_matrix_numpy,
    _compute_ellipse_coords_numba,
    _compute_ellipse_coords_numpy,
    _ea_to_pa_numba,
    _ea_to_pa_numpy,
    _harmonic_model_numba,
    _harmonic_model_numpy,
...
def test_numba_full_fit_accuracy():
    ...
    image, true_profile, params = create_sersic_model(R_e=R_e, n=n, I_e=I_e, eps=eps, pa=pa, oversample=10)
...
def test_numba_qa_figure():
    ...
    image, true_profile, params = create_sersic_model(R_e=R_e, n=n, I_e=I_e, eps=eps, pa=pa, oversample=12)
    ...
    if failed.any():
        ax_sb.scatter(
            x_axis[failed], np.log10(intens[failed]), marker="x", color="red", s=40, alpha=0.7, label="Failed", zorder=1
        )
    ...
    assert max_rel_diff < 0.02, f"Max relative difference {max_rel_diff * 100:.2f}% exceeds 2% threshold"
