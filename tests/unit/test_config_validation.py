...
def test_error_template_and_template_isophotes():
    ...
    template = [
        {"sma": 0, "x0": 32.0, "y0": 32.0, "eps": 0.2, "pa": 0.0, "intens": 100.0},
        {"sma": 5.0, "x0": 32.0, "y0": 32.0, "eps": 0.2, "pa": 0.0, "intens": 90.0},
    ]
...
def test_warn_geometry_convergence_impossible():
    ...
    with pytest.warns(UserWarning, match="geometry convergence can never trigger"):
        IsosterConfig(geometry_convergence=True, maxit=12, minit=10, geometry_stable_iters=5)
...
def test_no_warn_geometry_convergence_possible():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        IsosterConfig(geometry_convergence=True, maxit=50, minit=10, geometry_stable_iters=3)
