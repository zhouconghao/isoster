...
def _make_results(config_kwargs=None):
    ...
    isophotes = [
        {'sma': 5.0, 'intens': 100.0, 'intens_err': 1.0,
         'eps': 0.2, 'pa': 0.5, 'x0': 32.0, 'y0': 32.0,
         'rms': 2.0, 'stop_code': 0, 'niter': 15},
        {'sma': 10.0, 'intens': 50.0, 'intens_err': 0.8,
         'eps': 0.25, 'pa': 0.6, 'x0': 32.1, 'y0': 31.9,
         'rms': 1.5, 'stop_code': 0, 'niter': 12},
        {'sma': 15.0, 'intens': 20.0, 'intens_err': 1.5,
         'eps': 0.3, 'pa': 0.7, 'x0': 32.0, 'y0': 32.0,
         'rms': 3.0, 'stop_code': 2, 'niter': 50},
    ]
...
class TestFitsWriter:
    ...
    def test_no_hierarch_warnings(self, tmp_path):
        ...
        assert len(verify_warnings) == 0, (
            f"Got {len(verify_warnings)} VerifyWarning(s): "
            + "; ".join(str(w.message) for w in verify_warnings)
        )
...
class TestFitsReaderBackwardCompat:
    ...
    def test_legacy_returns_isophotes_and_none_config(self, tmp_path):
        ...
        isophotes = [
            {'sma': 5.0, 'intens': 100.0, 'eps': 0.2, 'pa': 0.5,
             'x0': 32.0, 'y0': 32.0, 'stop_code': 0, 'niter': 10},
        ]
