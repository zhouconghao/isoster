...
from isoster.sampling import IsophoteData, extract_isophote_data


def _make_sersic_image(size=128, n=2.0, r_eff=30.0, amplitude=1000.0, noise_sigma=5.0,
                       seed=42):
...
class TestIsophoteDataVariancesField:
    ...
    def test_variances_nan_filtering(self):
        ...
        data_with_var = extract_isophote_data(image, None, 32, 32, 20, 0.2, 0.0,
                                              variance_map=var_map)
...
class TestByteIdentity:
    ...
    def test_fit_image_byte_identity(self):
        ...
        config = IsosterConfig(
            x0=32.0, y0=32.0, sma0=10.0, minsma=5.0, maxsma=25.0,
            eps=0.2, pa=0.5, maxit=10
        )
...
class TestWLSCorrectness:
    ...
    def test_wls_recovers_truth_better(self):
        ...
        A = np.column_stack([
            np.ones_like(phi), np.sin(phi), np.cos(phi),
        ...
        assert wls_error < ols_error, (
            f"WLS error ({wls_error:.4f}) should be less than OLS ({ols_error:.4f})"
        )
...
class TestCosmicRayDownWeighting:
    ...
    def test_high_variance_outliers_downweighted(self):
        ...
        coeffs_clean, _ = fit_first_and_second_harmonics(phi, intens_clean,
                                                          variances=variances_clean)
        coeffs_cr, _ = fit_first_and_second_harmonics(phi, intens_cr,
                                                       variances=variances_cr)
        coeffs_ols_cr, _ = fit_first_and_second_harmonics(phi, intens_cr)

        # WLS with cosmic rays should still recover ~100
        assert abs(coeffs_cr[0] - true_mean) < 2.0, (
            f"WLS mean {coeffs_cr[0]:.1f} far from truth {true_mean}"
        )
        # OLS without variance info should be biased high
        assert abs(coeffs_ols_cr[0] - true_mean) > 10.0, (
            f"OLS mean {coeffs_ols_cr[0]:.1f} unexpectedly close to truth"
        )
...
class TestExactCovariance:
    ...
    def test_wls_covariance_no_scaling(self):
        ...
        np.testing.assert_allclose(actual_var_intercept, expected_var_intercept,
                                   rtol=0.05)
    ...
    def test_compute_parameter_errors_exact_mode(self):
        ...
        errs = compute_parameter_errors(
            phi, intens, 32, 32, 20, 0.2, 0.5, -5.0,
            cov_matrix=cov, coeffs=coeffs, use_exact_covariance=True
        )
...
class TestVarianceWeightedGradient:
    ...
    def test_gradient_current_data_with_variances(self):
        ...
        data = extract_isophote_data(image, None, 32, 32, 15, 0.2, 0.0,
                                     variance_map=var_map)
...
class TestHigherHarmonicsWLS:
    ...
    def test_simultaneous_wls_returns_dict(self):
        ...
        result = fit_higher_harmonics_simultaneous(
            phi, intens, 20.0, -5.0, [3, 4], variances=variances
        )
    ...
    def test_compute_deviations_wls(self):
        ...
        a, b, a_err, b_err = compute_deviations(phi, intens, 20.0, -5.0, 4,
                                                  variances=variances)
...
class TestFitIsophoteWithVariance:
    ...
    def test_fit_isophote_runs_with_variance_map(self):
        ...
        result = fit_isophote(image, None, 20.0, geometry, config,
                              variance_map=var_map)
    ...
    def test_fit_isophote_wls_smaller_intens_err(self):
        ...
        result_wls = fit_isophote(image, None, 20.0, geometry, config,
                                  variance_map=var_map)
...
class TestFitImageWithVariance:
    ...
    def test_fit_image_with_variance_map(self):
        ...
        image, var_map = _make_sersic_image(size=128, noise_sigma=5.0)
        config = IsosterConfig(
            x0=64, y0=64, sma0=10, minsma=5, maxsma=40,
            eps=0.2, pa=0.5, maxit=10
        )
