...
def compute_profile_agreement(iso_isoster, iso_photutils, R_e):
    ...
    intens_phu_interp = interp1d(sma_phu[mask_phu], intens_phu[mask_phu],
                                  kind='linear', bounds_error=False, fill_value=np.nan)
    eps_phu_interp = interp1d(sma_phu[mask_phu], eps_phu[mask_phu],
                               kind='linear', bounds_error=False, fill_value=np.nan)
    pa_phu_interp = interp1d(sma_phu[mask_phu], pa_phu[mask_phu],
                              kind='linear', bounds_error=False, fill_value=np.nan)
...
class TestPhotutilsComparison:
    ...
    @pytest.mark.parametrize("n,R_e,eps,pa,snr", [
        # Circular cases
    ...
    def test_isoster_vs_photutils(self, n, R_e, eps, pa, snr):
        ...
        # Run photutils
        isophotes_phu = run_photutils_isophote(image, x0, y0, eps, pa,
                                                sma0=10.0, minsma=3.0, maxsma=8*R_e)
        ...
        if eps > 0.1 and not np.isnan(agreement['pa_median_diff']):
            assert agreement['pa_median_diff'] < 5.0, \
                f"PA median difference {agreement['pa_median_diff']:.2f}° >= 5°"
